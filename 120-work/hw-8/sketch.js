...
function draw() {
    ...
    const exp = sin(frameCount*(PI/300)) ;
...
function Cube(bounds) {
    //make a point cloud
    let point = [
        createVector(-bounds, bounds, bounds),  //front upper left
        createVector(bounds, bounds, bounds),   //front upper right
        createVector(bounds, -bounds, bounds),  //front lower left
        createVector(-bounds, -bounds, bounds), //front lower right

        createVector(-bounds, bounds, -bounds), //rear upper left
        createVector(bounds, bounds, -bounds),  //rear upper right
        createVector(bounds, -bounds, -bounds), //rear lower left
        createVector(-bounds, -bounds, -bounds) //rear lower right
    ];

    //convert the points to 1x3 matrix
    //TODO: this is not needed. Write the pointcloud as matrix instead
    let matrix = [] ;
